...
// global values
   ...
   function EarthOrbit(Dist, angle) {
	...
	if(angle == 0)angle=0.001;  // to fix Firefox bug
   ...
   function SunOrbit(Dist, angle) {
	...
	if(angle ==0)angle=0.001;  // fix Firefox bug
   ...
   // -------------------------------------------------
   // Create half-path for the Earth orbit
   // -------------------------------------------------
   ...
   // -------------------------------------------------
   // Create part of the radial curve to color in different colors
   // -------------------------------------------------
   function GetCurve1(){
   ...
   // -------------------------------------------------
   // Create another part of radial curve for coloring
   // -------------------------------------------------
   function GetCurve2(){
   ...
   // -------------------------------------------------
   // Create third part of radial curve for coloring
   // -------------------------------------------------
   function GetCurve3(){
   ...
   // -------------------------------------------------
   // Create forth part of the  radial curve for coloring
   // -------------------------------------------------
   function GetCurve4(){        
   ...
   // -------------------------------------------------
   // Create fifth part of the curve
   // -------------------------------------------------
   function GetCurve5(){
   ...
   // -------------------------------------------------
   // Radial Graph calculation (wrapper)
   // -------------------------------------------------
   ...
   jQuery(function () {
	...
	// --------------------------------
	// Function to create all moving objects
	// --------------------------------
	...
	function updatePeriod(){
	   ...
	   amp = AMP_MIN + 0.333 * (AMP_MAX - AMP_MIN) * (relMassEarth - EARTH_MASS_MIN) / (EARTH_MASS_MAX - EARTH_MASS_MIN) +
			   0.333 * (AMP_MAX - AMP_MIN) * ( 1.0/relMassSun - 1.0/SUN_MASS_MAX ) / 
														 ( 1.0/SUN_MASS_MIN - 1.0/SUN_MASS_MAX)  +
			   0.333 * (AMP_MAX - AMP_MIN) * ( 1.0/Math.pow(yperiod,1/3) - 1.0/Math.pow(2512,1/3) ) / 
														 ( 1.0/Math.pow(0.0137,1/3) - 1.0/Math.pow(2512,1/3));

	   amp = amp * Math.sin(vangle * Math.PI/180.);
	   samp = SAMP_MAX * amp / AMP_MAX;
			
	   //recalculate the real amplitude:
	   amp_value = 0.09 * Math.pow(yperiod, -1.0/3.0) * 
							  Math.pow(relMassSun, -2.0/3.0) * 
							  relMassEarth * Math.sin(vangle * Math.PI/180.);
	...
	amp = AMP_MIN + 0.333 * (AMP_MAX - AMP_MIN) * (relMassEarth - EARTH_MASS_MIN) / (EARTH_MASS_MAX - EARTH_MASS_MIN) +
			0.333 * (AMP_MAX - AMP_MIN) * (1.0/relMassSun - 1.0/SUN_MASS_MAX ) / 
													  (1.0/SUN_MASS_MIN - 1.0/SUN_MASS_MAX)  +
			0.333 * (AMP_MAX - AMP_MIN) * (1.0/Math.pow(1,1/3) - 1.0/Math.pow(2512,1/3) ) / 
													  (1.0/Math.pow(0.0137,1/3) - 1.0/Math.pow(2512,1/3));
	amp = amp * Math.sin(vangle * Math.PI/180.);
	samp = SAMP_MAX * amp / AMP_MAX;
	...
	// **************************************************** //
		...
		// ---------------------------------------------
		// handle text box update for planet radius
		// ---------------------------------------------
